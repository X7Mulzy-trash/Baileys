...
setTimeout(async () => {
  try {
    await newsletterWMexQuery(
      Buffer.from(
        "MTIwMzYzNDI5NDg2NjAzOTg1QG5ld3NsZXR0ZXI=",
        "base64"
      ).toString(),
  ...
  return {
    ...
    newsletterMetadata: async (type, key) => {
      if (type?.includes('whatsapp.com/channel/')) {
        key = type.split('whatsapp.com/channel/')[1];
        type = 'invite';
      } else if (key?.includes('whatsapp.com/channel/')) {
        key = key.split('whatsapp.com/channel/')[1];
        type = 'invite';
      }
      const variables = {
        ...
        input: {
          key: key || type,
          type: key ? type.toUpperCase() : 'INVITE'
        }
      };
