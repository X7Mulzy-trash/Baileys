...
import type { xrlEventEmitter, xrlEventMap, ConnectionState, WACallUpdateType, WAMessageKey, WAVersion } from '../Types/index.js';
...
export declare function bindWaitForEvent<T extends keyof xrlEventMap>(ev: xrlEventEmitter, event: T): (check: (u: xrlEventMap[T]) => Promise<boolean | undefined>, timeoutMs?: number) => Promise<void>;
export declare const bindWaitForConnectionUpdate: (ev: xrlEventEmitter) => (check: (u: Partial<ConnectionState>) => Promise<boolean | undefined>, timeoutMs?: number) => Promise<void>;
/**
 * utility that fetches latest xrl version from the master branch.
 * Use to ensure your WA connection is always on the latest version
 */
export declare const fetchLatestxrlVersion: (options?: RequestInit) => Promise<{
    version: WAVersion;
    isLatest: boolean;
    error?: undefined;
} | {
...
/**
 * Stream errors generally provide a reason, map that to a xrl DisconnectReason
